...
from hyperopt import hp, fmin, tpe, atpe, Trials, space_eval, STATUS_OK
...
from s2and.eval import pairwise_precision_recall_fscore, kmetric_precision_recall_fscore
...
class Clusterer:
    ...
    def optimize_block_eps_b3(self, block_key: str, signatures: List[str], 
                          dataset: "ANDData", true_clusters: Dict[str, List[str]], 
                          n_iter: int = 20) -> Dict[str, float]:
        """
        Optimizes the eps value for a single block.
        
        Returns
        -------
        Dict[str, float]:
            A dictionary containing 'eps', 'f1', 'precision', and 'recall'
        """
        
        relevant_true_clusters = {
            k: v for k, v in true_clusters.items() 
            if any(sig in signatures for sig in v)
        }
        
        block_dict = {block_key: signatures}
        distance_matrices = self.make_distance_matrices(block_dict, dataset)
        ...
        def objective(params):
            ...
            try:
                precision, recall, f1, _, _, _ = b3_precision_recall_fscore(
                    relevant_true_clusters,
                    pred_clusters
                )

                if f1 > best_metrics['f1']:
                    best_metrics['eps'] = params['eps']
                    best_metrics['f1'] = f1
        ...
        best_metrics['eps'] = best['eps']
        
        return best_metrics

    def optimize_block_eps_pairwise(self, block_key: str, signatures: List[str], 
                                  dataset: "ANDData", true_clusters: Dict[str, List[str]], 
                                  n_iter: int = 20, metric: str = "cmacro") -> Dict[str, float]:
        ...
        signatures : List[str]
            List of signatures to be clustered
        dataset : ANDData
            Dataset containing features
        true_clusters : Dict[str, List[str]]
            True cluster assignments in the format {cluster_id: [signature_list]}
        n_iter : int, optional
            Number of optimization iterations (default: 20)
        metric : str, optional
            Evaluation metric, either "cmacro" or "clusters" (default: "cmacro")
        
        Returns
        -------
        Dict[str, float]:
            Dictionary containing 'eps', 'f1', 'precision', and 'recall'
        """
        
        if metric not in ["cmacro", "clusters"]:
            raise ValueError("metric must be either 'cmacro' or 'clusters'")
        ...
        try:
            distance_matrices = self.make_distance_matrices(block_dict, dataset)
            logger.debug(f"Successfully created distance matrices for block {block_key}")
        except Exception as e:
            logger.error(f"Error creating distance matrices for block {block_key}: {str(e)}")
            return {
                'eps': 0.5,
                'f1': 0.0,
                'precision': 0.0,
        ...
        # block's ground truth
        block_true_clusters = dataset.construct_cluster_to_signatures(block_dict)
        logger.debug(f"Block {block_key} - True clusters structure: {block_true_clusters}")
        ...
        def objective(params):
            ...
            try:
                ...
                if not pred_clusters:
                    logger.debug(f"Block {block_key}: Empty prediction result for eps={params['eps']}")
                    return {'loss': float('inf'), 'status': STATUS_OK}
                
                logger.debug(f"Block {block_key} - Prediction result structure: {pred_clusters}")
                
                try:
                    precision, recall, f1 = pairwise_precision_recall_fscore(
                        block_true_clusters,
                        pred_clusters,
                        block_dict,
                        metric
                    ...
                    logger.debug(f"Block {block_key} - Metrics for eps={params['eps']}: "
                               f"P={precision:.3f}, R={recall:.3f}, F1={f1:.3f}")
                    
                    if f1 > best_metrics['f1']:
                        best_metrics['eps'] = float(params['eps'])
                        best_metrics['f1'] = float(f1)
                        best_metrics['precision'] = float(precision)
                        best_metrics['recall'] = float(recall)
                        logger.info(f"Block {block_key} - New best metrics found: "
                                  f"eps={params['eps']:.3f}, F1={f1:.3f}")
                    
                    return {'loss': -f1, 'status': STATUS_OK}
                    
                except Exception as e:
                    logger.error(f"Block {block_key} - Error calculating metrics: {str(e)}")
                    return {'loss': float('inf'), 'status': STATUS_OK}
                    
            except Exception as e:
                logger.error(f"Block {block_key} - Error in prediction: {str(e)}")
                return {'loss': float('inf'), 'status': STATUS_OK}
        ...
        try:
            ...
            if best and 'eps' in best:
                best_metrics['eps'] = float(best['eps'])
                logger.info(f"Block {block_key} - Optimization completed successfully: "
                           f"best_eps={best_metrics['eps']:.3f}, "
                           f"best_f1={best_metrics['f1']:.3f}")
            else:
                logger.warning(f"Block {block_key} - Optimization completed but no best parameters found")
                
        except Exception as e:
            logger.error(f"Block {block_key} - Optimization failed: {str(e)}")
            
        if best_metrics['f1'] == -float('inf'):
            logger.warning(f"Block {block_key} - No valid results found, returning default metrics")
            best_metrics['f1'] = 0.0
            
        return best_metrics


    def optimize_block_eps_kmetric(self, block_key: str, signatures: List[str], 
                                dataset: "ANDData", true_clusters: Dict[str, List[str]], 
                                n_iter: int = 20) -> Dict[str, float]:
        """
        Optimizes the eps parameter for a specified block using the K-metric.
        ...
        block_key : str
            Identifier of the block
        signatures : List[str]
            List of signatures contained in the block
        dataset : ANDData
            Dataset object containing features
        true_clusters : Dict[str, List[str]]
            Ground truth clusters in the format {cluster_id: [signature1, signature2, ...]}
        n_iter : int, optional
            Number of optimization iterations (default: 20)
        ...
        -------
        Dict[str, float]
            Dictionary containing optimal eps with corresponding K, ACP, and AAP values
        """
        
        try:
            if len(signatures) <= 1:
                logger.info(f"Block {block_key} has {len(signatures)} signatures, skipping optimization")
                return {'eps': 1.0, 'k': 1.0, 'acp': 1.0, 'aap': 1.0}
            
            if not true_clusters:
                logger.warning(f"No true clusters found for block {block_key}")
                return {'eps': 0.5, 'k': -float('inf'), 'acp': -float('inf'), 'aap': -float('inf')}
            
            logger.debug(f"Block {block_key}:")
            logger.debug(f"- Signatures in block: {len(signatures)}")
            logger.debug(f"- Number of relevant true clusters: {len(true_clusters)}")
            logger.debug(f"- True clusters sizes: {[len(sigs) for sigs in true_clusters.values()]}")
    
            # 计算距离矩阵
            distance_matrices = self.make_distance_matrices({block_key: signatures}, dataset)
            if not distance_matrices:
                logger.error(f"Failed to compute distance matrices for block {block_key}")
                return {'eps': 0.5, 'k': -float('inf'), 'acp': -float('inf'), 'aap': -float('inf')}
            ...
            def objective(params):
                try:
                    ...
                    pred_clusters_dict, _ = self.predict(
                        block_dict={block_key: signatures},
                        dataset=dataset,
                        dists=distance_matrices,
                        cluster_model_params=cluster_params
                    ...
                    if not pred_clusters_dict:
                        logger.warning(f"No clusters predicted for eps={eps}")
                        return {'loss': float('inf'), 'status': STATUS_OK}
                    
                    logger.debug(f"Eps={eps:.3f}:")
                    logger.debug(f"- Number of predicted clusters: {len(pred_clusters_dict)}")
                    logger.debug(f"- Predicted clusters sizes: {[len(c) for c in pred_clusters_dict.values()]}")
                    ...
                    if not np.isfinite(k):
                        logger.warning(f"Invalid K value for eps={eps}")
                        return {'loss': float('inf'), 'status': STATUS_OK}
                        
                    logger.debug(f"- K: {k:.3f}, ACP: {acp:.3f}, AAP: {aap:.3f}")
                    ...
                    return {'loss': -k, 'status': STATUS_OK}
            ...
            if best_metrics['eps'] is None:
                best_metrics['eps'] = best['eps']
                
            logger.info(f"Block {block_key} optimization completed:")
            logger.info(f"- Best eps: {best_metrics['eps']:.3f}")
            logger.info(f"- Best K: {best_metrics['k']:.3f}")
            logger.info(f"- Corresponding ACP: {best_metrics['acp']:.3f}")
            logger.info(f"- Corresponding AAP: {best_metrics['aap']:.3f}")
                
            return best_metrics
                
        except Exception as e:
            logger.error(f"Error in optimize_block_eps_k for block {block_key}: {str(e)}")
            return {
                'eps': 0.5,
                'k': -float('inf'),
                'acp': -float('inf'),
