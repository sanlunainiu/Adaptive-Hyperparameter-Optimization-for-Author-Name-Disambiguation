...
class ANDData:
    ...
    def split_blocks_helper(
        ...
        for block_id, signatures in blocks_dict.items():
            print(f"Block {block_id}: {len(signatures)} signatures")
    ...
    def split_data_signatures_fixed_my(
        self,
        blocks: Dict[str, List[str]]
        ) -> Tuple[Dict[str, List[str]], Dict[str, List[str]], Dict[str, List[str]]]:
        """
        Uses predefined signature splits without performing random partitioning.
        """
        
        train_block_dict = self.group_signature_helper(self.train_signatures)
        test_block_dict = self.group_signature_helper(self.test_signatures)
        val_block_dict = {}
